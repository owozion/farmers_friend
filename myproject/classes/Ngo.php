    ...
    class Ngo extends Db
    {
        ...
        public function __construct(){
            $this->dbconn = $this->connect();
               ...
               //update db
        public function update_ngo($fname, $lname, $profilepic, $state, $lga, $id){
                ...
                copy("../uploads/$newname","../admin/uploads/$newname"); //add to admin folder so admin can see images too when he fetches
                  ...
                  //update db
               $sql = "UPDATE ngo SET ngo_firstname=?, ngo_lastname=?, ngo_profilep=?, state_id=?, lga_id=?  WHERE ngo_id=?";
               $stmt = $this->dbconn->prepare($sql);
               $chk = $stmt->execute([$fname, $lname, $newname, $state, $lga, $id]);
            if($chk){
                return true;
        ...
        public function login($email,$password){
            try{
                $query = "SELECT * FROM ngo WHERE ngo_email=?";
                $stmt = $this->dbconn->prepare($query);
                $stmt->execute([$email]);
                $result = $stmt->fetch(PDO::FETCH_ASSOC);
                if($result){
                    $hashed = $result['ngo_password'];
                    $chk = password_verify($password,$hashed); //true or false
                    if($chk){
                        return $result['ngo_id'];
                    }else{
        ...
        public function get_ngo_by_id($id){
            try{
                $query = "SELECT * FROM ngo WHERE ngo_id =?";
                $stmt = $this->dbconn->prepare($query);
                $stmt->execute([$id]);
                $result = $stmt->fetch(PDO::FETCH_ASSOC);
                return $result;
        ...
        public function insert_ngo($firstname,$lastname,$email,$pwd,$category){
           try{
            #sql
            $query = "INSERT INTO ngo(ngo_firstname, ngo_lastname, ngo_email, ngo_password, usercat_id) VALUES (?,?,?,?,?)";
            #prepare
            $stmt = $this->dbconn->prepare($query);
            #execute
